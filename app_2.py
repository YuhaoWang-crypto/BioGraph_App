...
def load_data():
    """
    读取预处理好的 CSV 文件。
    要求 CSV 必须包含: Gene_Symbol, Cluster, UMAP_X, UMAP_Y, Real_Protein_HalfLife_Hours 等
    """
    # 尝试读取文件
    try:
        # 这里的文件名必须和你上传到 GitHub 的文件名一致
        ...
        st.error("❌ 未找到数据文件！请确保 'final_analysis_result.csv' 已上传到项目根目录。")
        return pd.DataFrame()
...
with tab1:
    ...
    with col1:
        color_col = st.radio("上色依据:", ['Cluster', 'Stability_Level', 'N_Term_AA', 'Is_Cancer'], index=0)
        
        # 尝试自动生成 Is_Cancer 如果不存在
        if color_col == 'Is_Cancer' and 'Is_Cancer' not in df_filtered.columns:
            df_filtered['Is_Cancer'] = df_filtered['cc_function'].str.contains('cancer|tumor', case=False).map({True:'Yes', False:'No'})

    with col2:
        # 高亮逻辑
        sizes = [5] * len(df_filtered)
        colors = df_filtered[color_col]
        
        if search_gene:
            # 如果搜索了基因，把它变大
            highlight_mask = df_filtered['Gene_Symbol'] == search_gene
            if highlight_mask.any():
                st.success(f"找到基因: {search_gene}")
            else:
                st.warning(f"未在筛选结果中找到: {search_gene}")
...
with tab2:
    ...
    if selected_gene_tab2:
        ...
        with c1:
            ...
            st.info(f"所属 Cluster: {row['Cluster']}")
...
with tab3:
    ...
    st.write("输入关键词（如 mitochondrial, kinase），构建基于半衰期相似性的共现网络。")
...
# 底部
st.markdown("---")
st.caption("BioGraph App v2.0 | Powered by Streamlit & UniProt")
